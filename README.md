...
### 0. FizzBuzz — `challenge/0-fizzbuzz.py`
...
**Bug 1:** The password setter used `self._password` (single underscore) instead of `self.__password` (double underscore). Python's name mangling means these are different attributes, so the password was never stored under the private `__password` attribute.
...
### 4. Double linked list — `challenge/4-delete_dnodeint/delete_dnodeint_at_index.c`
